...
export async function EditableTaskArchiveRoute({
  ...
  if (task === 'mediaDistribution' && !posts.length) {
    const feed = await fetchSiteFeed(48, { fresh: true, category: category === 'all' ? undefined : category })
    posts = (feed?.posts || []).filter((post) => asText(getContent(post).type).toLowerCase() !== 'comment').slice(0, 24)
    pagination = {
      page: 1,
      limit: 24,
      total: posts.length,
      totalPages: Math.max(1, Math.ceil(posts.length / 24)),
      hasPrevPage: false,
      hasNextPage: false,
    }
  }
...
function EditorialArchive({
  ...
  const hrefFor = (post: SitePost) => {
    const actualTask = task === 'mediaDistribution' ? (getPostTaskKey(post) as TaskKey | null) : null
    return buildPostUrl(actualTask || task, post.slug)
  }
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        <section className="bg-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-14">
            <div>
              ...
              <h1 className="editorial-serif mt-3 text-5xl font-black leading-tight sm:text-6xl lg:text-7xl">
            ...
            <p className="max-w-md border-l-4 border-[var(--slot4-accent)] pl-5 text-sm font-bold leading-7 text-black/65">
              Real media distribution posts, releases, announcements, and coverage notes organized for fast discovery.
            </p>
          </div>
        </section>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[var(--editable-container)] overflow-hidden rounded-md bg-white shadow-[0_24px_70px_rgba(39,49,44,.08)] lg:grid-cols-[1.75fr_0.75fr]">
            <Link href={hrefFor(lead)} className="group relative min-h-[34rem] overflow-hidden border-b border-black/10 lg:border-b-0 lg:border-r lg:border-black/10">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                ...
                <h2 className="editorial-serif mt-5 max-w-4xl text-4xl font-black leading-tight sm:text-6xl">{lead.title}</h2>
            ...
            <div className="grid">
              ...
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={hrefFor(post)} className="group grid grid-cols-[7rem_1fr] border-b border-black/10 bg-white last:border-b-0">
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b border-black/10 pb-4">
            <h2 className="editorial-serif text-4xl font-black sm:text-5xl">All media posts</h2>
            <form action={basePath} className="flex rounded-full border border-black/10 bg-white p-1">
              ...
              <button className="h-11 rounded-full bg-[var(--slot4-dark-bg)] px-5 text-xs font-black uppercase tracking-[0.14em] text-white">Filter</button>
            </form>
          </div>
          ...
          {remaining.length ? (
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={hrefFor(post)} className="group overflow-hidden rounded-md border border-black/10 bg-white transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_48px_rgba(39,49,44,.12)]">
