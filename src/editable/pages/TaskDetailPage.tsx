  ...
  return (
    <EditableSiteShell>
      <main style={detailVars} className="bg-[var(--detail-bg)] text-[var(--detail-text)]">
        ...
        {task === 'article' || task === 'mediaDistribution' ? <ArticleDetail task={task} post={post} related={related} comments={comments} brandName={brandName} /> : null}
      </main>
    </EditableSiteShell>
  )
}
...
function ArticleDetail({ task, post, related, comments, brandName }: { task: TaskKey; post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }>; brandName: string }) {
    ...
    <section className="bg-[var(--slot4-page-bg)]">
      <header>
        <div className="mx-auto max-w-[1180px] px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
          <BackLink task={task} />
          <div className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-black/10 pt-4 text-[11px] font-black uppercase tracking-[0.16em]">
            <span className="rounded-full bg-[var(--slot4-accent)] px-3 py-2 text-white">{categoryOf(post, 'News')}</span>
            <span className="text-black/45">Media distribution</span>
          </div>
          <h1 className="editorial-serif mt-6 max-w-6xl text-5xl font-black leading-tight sm:text-6xl lg:text-[5rem]">{post.title}</h1>
        </div>
      </header>

      {images[0] ? (
        <figure className="mx-auto max-w-[1180px] overflow-hidden rounded-md bg-white shadow-[0_24px_70px_rgba(39,49,44,.08)]">
      ...
      <div className="mx-auto grid max-w-[1180px] gap-12 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,760px)_300px] lg:px-8 lg:py-16">
        <article className="min-w-0 rounded-md bg-white p-6 shadow-[0_18px_50px_rgba(39,49,44,.07)] sm:p-9">
          <BodyContent post={post} />
          <EditableComments slug={post.slug} comments={comments} />
        </article>
        <div>
          <RelatedPanel task={task} post={post} related={related} brandName={brandName} />
        </div>
      </div>
    </section>
  )
...
function RelatedPanel({ task, post, related, compact = false, brandName = process.env.NEXT_PUBLIC_SITE_NAME || SITE_CONFIG.name }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean; brandName?: string }) {
  const taskConfig = getTaskConfig(task)
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-md bg-[var(--slot4-dark-bg)] p-5 text-white">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-white/55">Distribution</p>
          <div className="mt-4 grid gap-3 text-sm font-bold opacity-75">
            <p className="inline-flex items-center gap-2"><Tag className="h-4 w-4" /> Task: {taskConfig?.label || task}</p>
            <p className="inline-flex items-center gap-2"><CheckCircle2 className="h-4 w-4" /> Site: {brandName}</p>
          </div>
        </div>
      ) : null}
      {related.length ? (
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-12 rounded-md border border-black/10 bg-[var(--slot4-panel-bg)] p-5">
