...
export function RailPostCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block rounded-md border border-black/10 bg-[var(--slot4-surface-bg)] p-5 ${dc.motion.lift}`}>
      ...
      <h3 className="mt-5 line-clamp-3 text-xl font-black leading-[1.08]">{post.title}</h3>
      <p className="mt-4 line-clamp-4 text-sm leading-6 text-black/58">{getEditableExcerpt(post, 132)}</p>
      <span className="mt-6 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[.12em] text-[var(--slot4-gold)]">Read update <ArrowRight className="h-4 w-4" /></span>
    </Link>
  )
}
...
export function CompactIndexCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className="group grid min-w-0 grid-cols-[46px_1fr] gap-4 border-t border-black/10 py-5 first:border-t-0">
      <span className="editorial-serif text-3xl font-black leading-none text-[var(--slot4-accent)]">{String(index + 1).padStart(2, '0')}</span>
      <div className="min-w-0">
        ...
        <p className="mt-2 line-clamp-2 text-xs leading-5 text-black/50">{getEditableExcerpt(post, 90)}</p>
      </div>
    </Link>
  )
}
